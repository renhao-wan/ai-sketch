...
export default function ZoomToolbar({ scale, onZoomIn, onZoomOut, onFitToView }: ZoomToolbarProps) {
  return (
    <div className="absolute top-3 right-3 z-20 flex items-center gap-1 bg-[var(--bg-glass)] backdrop-blur-sm rounded-lg border border-[var(--border)] shadow-[var(--shadow-soft)] p-1">
      <button onClick={onZoomOut} className="p-1.5 hover:bg-[var(--surface-warm-hover)] rounded transition-colors" title="缩小">
      ...
      <button onClick={onZoomIn} className="p-1.5 hover:bg-[var(--surface-warm-hover)] rounded transition-colors" title="放大">
        <ZoomIn size={14} className="text-[var(--muted)]" />
      </button>
      <div className="w-px h-4 bg-[var(--border)] mx-0.5" />
      <button onClick={onFitToView} className="p-1.5 hover:bg-[var(--surface-warm-hover)] rounded transition-colors" title="适应视图">
