...
export default function ConfigManager({ isOpen, onClose, onConfigSelect }) {
  ...
  const loadConfigs = () => {
    ...
    catch (err) { setError('加载配置失败: ' + err.message); }
  };

  const handleCreateNew = () => { setIsCreating(true); setEditingConfig({ name: '', type: 'openai', baseUrl: '', apiKey: '', model: '', description: '' }); };
  const handleEdit = (config) => { setIsCreating(false); setEditingConfig({ ...config }); };

  const handleDelete = async (configId) => {
    setConfirmDialog({ isOpen: true, title: '确认删除', message: '确定要删除这个配置吗？此操作不可恢复。',
      onConfirm: async () => { try { await configManager.deleteConfig(configId); loadConfigs(); setError(''); setNotification({ isOpen: true, title: '删除成功', message: '配置已成功删除', type: 'success' }); } catch (err) { setError('删除配置失败: ' + err.message); } },
    });
  };

  const handleClone = (config) => { try { configManager.cloneConfig(config.id, `${config.name} (副本)`); loadConfigs(); setError(''); } catch (err) { setError('克隆配置失败: ' + err.message); } };

  const handleSetActive = async (configId) => {
    try { await configManager.setActiveConfig(configId); loadConfigs(); onConfigSelect?.(configManager.getActiveConfig()); setError(''); }
    catch (err) { setError('切换配置失败: ' + err.message); }
  };
  ...
  const handleTestConnection = async (config) => {
    ...
    } catch (err) { setNotification({ isOpen: true, title: '连接测试失败', message: err.message, type: 'error' }); }
  ...
  const handleSaveConfig = (configData) => {
    try {
      ...
      else { configManager.updateConfig(editingConfig.id, configData); if (editingConfig.id === activeConfigId) onConfigSelect?.(configManager.getConfig(editingConfig.id)); }
      setEditingConfig(null); setIsCreating(false); loadConfigs(); setError('');
    } catch (err) { setError('保存配置失败: ' + err.message); }
  };
  ...
  const handleExport = () => {
    ...
    } catch (err) { setError('导出配置失败: ' + err.message); }
  };
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      catch (err) { setError('导入配置失败: ' + err.message); }
    };
    input.click();
  };
      ...
      <div className="relative bg-white/80 backdrop-blur-2xl rounded-3xl border border-white/15 shadow-[0_20px_60px_rgba(15,23,42,0.12)] w-full max-w-md max-h-[90vh] overflow-y-auto animate-slide-up">
        ...
        <div className="px-7 pb-6 space-y-4">
          ...
          <div>
            ...
            <select value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value, model: '' })} className={inputClass}>
