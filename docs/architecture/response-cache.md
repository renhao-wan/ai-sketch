...
CREATE TABLE response_cache (
  ...
  prompt_hash TEXT NOT NULL,        -- 历史遗留列，新数据与 id 相同
  format TEXT NOT NULL,             -- 历史遗留列，新数据为空
  chart_type TEXT NOT NULL,         -- 历史遗留列，新数据为空
...
CREATE INDEX idx_response_cache_hash ON response_cache(prompt_hash, format, chart_type);
...
> **注**：`prompt_hash`、`format`、`chart_type` 为历史遗留列，新数据不再使用。`id` 列直接存储统一生成的缓存键。保留这些列是为了兼容旧数据库。
