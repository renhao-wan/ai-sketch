...
const ACTION_PROMPTS: Record<AIActionType, string> = {
  layout: '你是图表布局专家。分析用户提供的图表代码，自动调整节点位置和间距，使图表更整齐易读。重要：必须保持与输入相同的代码格式（如 Excalidraw JSON、Mermaid、Draw.io XML），只返回修改后的完整代码，不要任何解释。',
  beautify: '你是图表美化专家。优化图表的视觉风格，包括颜色搭配、字体大小、对齐方式。重要：必须保持与输入相同的代码格式（如 Excalidraw JSON、Mermaid、Draw.io XML），只返回修改后的完整代码，不要任何解释。',
  simplify: '你是代码简化专家。精简图表代码结构，去除冗余元素，合并重复定义。重要：必须保持与输入相同的代码格式（如 Excalidraw JSON、Mermaid、Draw.io XML），只返回修改后的完整代码，不要任何解释。',
